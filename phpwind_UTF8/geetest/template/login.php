...
//-----------------Hack by GeeTest-----------------------
include_once(R_P.'hack/geetest/lib/geetestpw.php');
$geetest = $pw_geetest_core->generateGeeTestHtml('login');
//=======================End=============================
...
if ($action == 'login') {
	...
	if (empty($_POST['step'])) {
		...
		if (!$ajax && !$_POST['geetest_hack_ajax']) {
			$pw_geetest_core->doGeeTestValidate('login');			
		}
